...
function juju_client_prepare()
{
    ...
    OS_AUTH_PUBLIC=$(openstack endpoint list | grep keystone | grep public | awk '{print $14}')
    ...
    openstack:
        ...
        regions:
            $OS_REGION_NAME:
                endpoint: $OS_AUTH_PUBLIC' > clouds.yaml"
...
function juju_generate_metadata()
{
    ...
    cmd2="rsync -e 'ssh $ssh_args' -av mt ubuntu@$floating_ip_metadata:~/"
    exec_cmd_on_client $cmd2
...
function bootstrap_juju_controller()
{
    cmd="juju bootstrap openstack openstack \
        ...
        --verbose --debug"
    # encounter 404 error authentication failed after this step
    exec_cmd_on_client $cmd
}
