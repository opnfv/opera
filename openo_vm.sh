...
##############################################################################
set -ex
OPENO_VM_DIR=${WORK_DIR}/deploy/openo_vm
...
function launch_openo() {
    ...
    local old_iso=$OPENO_DIR/work/iso/${OPENO_VM_ISO}
    ...
    cp $OPENO_DIR/util/isolinux.cfg $new_mnt/isolinux/ -f
    cp $OPENO_DIR/util/ks.cfg $new_mnt/isolinux/ -f
    ...
    sed -e "s/REPLACE_MEM/$OPENO_VIRT_MEM/g" \
        ...
        $OPENO_DIR/util/openo.xml \
        > $OPENO_VM_DIR/libvirt.xml

    sudo virsh define $OPENO_VM_DIR/libvirt.xml
