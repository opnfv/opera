...
function package_prepare()
{
    if [ $(grep Ubuntu /etc/issue) ]; then
        ...
        sudu apt-get install -y wget mkisofs qemu-utils qemu-kvm libvirt-bin openvswitch-switch python-pip
    else
        sudo yum update -y
        sudo yum install -y wget mkisofs qemu-kvm libvirt-bin openvswitch-switch python-pip
    fi
