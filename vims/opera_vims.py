...
class BbbVnf(vnf_base.VnfOnBoardingBase):

    logger = ft_logger.Logger("VNF BBB").getLogger()

    def __init__(self):
        super(BbbVnf, self).__init__(case="bbb")
...
# TODO see how to use build in exception form releng module
    ...
    def main(self, **kwargs):
        self.logger.info("BBB VNF onboarding")
        self.execute()
...
if __name__ == '__main__':
    ...
    bbb_vnf = BbbVnf()
    try:
        result = bbb_vnf.main(**args)
        if result != testcase_base.TestcaseBase.EX_OK:
            sys.exit(result)
        if args['pushtodb']:
            sys.exit(bbb_vnf.push_to_db())
    except Exception:
